...
export function getBoxFilesDownloadLinks(documentIds, accessTokenString) {
    const boxUrls = documentIds.map(docId => `${BOX_API_DOWNLOAD_BASE_URL}${docId}?fields=${BOX_FIELD_DOWNLOAD_URL}`);
    const boxHeaders = {
    ...
    const promisesArray = [];
    boxUrls.map(boxUrl => {
        promisesArray.push(getHandledServiceRequest(
            {
                pluginName,
                serviceName
            }, (resolve, reject) => {
...
export function getDownloadAccessToken(documentId, env = "e2e") {
  ...
  `;
    const requestInfo = getRequestInfo(env, DOCUMENT_SERVICE_BASE_URL, "preprdakyresMiM27Q3l12Fcg81vHuH8fGm7d7Z5", "prdakyresh3dP1M4JldjgUssgx6YdZHSx5UJxb4c");
    return callHandleServiceRequest("iam-account-manager-ui", "mydata-fetch-download-access-token", requestInfo, query, "POST");
}
...
export function formatGetWorkOrdersResponse(res) {
  const workOrdersList = get(
    res,
    'data.dataGovernanceWorkOrderHolder.workOrders.edges',
    []
  );
  ...
  if (!workOrdersList.length) {
    ...
    const DELETE_STATE = [
      ...
      MYDATA_API_WORK_ORDER_STATE.WO_NON_CANCELLABLE
        .INTUIT_DELETE_CHECK_IN_PROGRESS,
    ...
    workOrdersList.forEach((order) => {
      ...
      if (requestType === REQUEST_TYPE.DELETE && DELETE_STATE.includes(state)) {
        ...
        if (
          !deleteOrder ||
          deleteOrder.cancelByUtcTime < node.cancelByUtcTime
        ) {
          deleteOrder = node;
        }
      } else if (
        requestType === REQUEST_TYPE.ACCESS &&
        DOWNLOAD_STATE.includes(state)
      ) {
        node.cancelByUtcTime = new Date(node.cancelByUtcTime);

        //Make sure we always get the latest one - TODO: refactor
        if (
          !downloadOrder ||
          downloadOrder.cancelByUtcTime < node.cancelByUtcTime
        ) {
          downloadOrder = node;
        }
      }
      ...
      node.scope.products = node.scope.products.map((product) => {
        return {
          productName: PRODUCT_NAME_MAPPING[product],
    ...
    if (downloadOrder && deleteOrder) {
      if (downloadOrder.cancelByUtcTime < deleteOrder.cancelByUtcTime) {
        //delete is the latest
        if (
          downloadOrder.state ===
          MYDATA_API_WORK_ORDER_STATE.COMPRESSION_COMPLETE
        ) {
          status = MYDATA_API_WORK_ORDER_STATE.WO_CANCELLABLE[deleteOrder.state]
            ? MYDATA_WO_STATUS.DELETE_IN_PROGRESS_DOWNLOAD_READY
            : MYDATA_WO_STATUS.DELETE_BLOCK_IN_PROGRESS_DOWNLOAD_READY;
        } else {
          status = MYDATA_API_WORK_ORDER_STATE.WO_CANCELLABLE[deleteOrder.state]
            ? MYDATA_WO_STATUS.DELETE_IN_PROGRESS
            : MYDATA_WO_STATUS.DELETE_BLOCK_IN_PROGRESS;
        }
      } else {
        status =
          downloadOrder.state ===
          MYDATA_API_WORK_ORDER_STATE.COMPRESSION_COMPLETE
            ? MYDATA_WO_STATUS.DOWNLOAD_READY
            : MYDATA_WO_STATUS.DOWNLOAD_IN_PROGRESS;
      }
    } else if (deleteOrder) {
      status = MYDATA_API_WORK_ORDER_STATE.WO_CANCELLABLE[deleteOrder.state]
        ? MYDATA_WO_STATUS.DELETE_IN_PROGRESS
      ...
      status =
        downloadOrder.state === MYDATA_API_WORK_ORDER_STATE.COMPRESSION_COMPLETE
          ? MYDATA_WO_STATUS.DOWNLOAD_READY
          : MYDATA_WO_STATUS.DOWNLOAD_IN_PROGRESS;
    }
...
export function formatProductList(productList, selectedFlagName) {
  return productList
    .filter((product) => product[selectedFlagName])
...
// TODO: move this product list to a file for localization
export const rawProductList = [
  ...
  },
  {
    ...
    productValue: 'TURBO',
    productDeleteDesc:
      "You'll lose access to 24/7 credit monitoring and your real-time debt-to-income ratio.",
    productDownloadDesc:
      "You'll get your personal profile data, tax return and refund information, and any personalized financial advice we provided.",
  },
  {
    productName: 'Mint',
    productId: 'mt',
    productValue: 'MINT',
    productDeleteDesc:
      "You'll lose access to your budgets, categorized spending history and goal tracking.",
    productDownloadDesc:
